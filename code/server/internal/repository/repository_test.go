...
func TestInMemoryFamilyRepository_CreateFamily(t *testing.T) {
	repo := memrepo.NewInMemoryFamilyRepository()
	...
	if retrieved.PairingCode != family.PairingCode {
		t.Errorf("retrieved.PairingCode = %v, want %v", retrieved.PairingCode, family.PairingCode)
	}
}
...
func TestInMemoryChildRepository_CreateChild(t *testing.T) {
	repo := memrepo.NewInMemoryChildRepository()
	ctx := context.Background()
	familyID := uuid.New()
	...
	if retrieved.Name != child.Name {
		t.Errorf("retrieved.Name = %v, want %v", retrieved.Name, child.Name)
	}
}
...
// Test isolation: verify mutations don't affect stored data
func TestInMemoryFamilyRepository_Isolation(t *testing.T) {
	...
	err := repo.CreateFamily(ctx, original)
	if err != nil {
		t.Fatalf("CreateFamily() error = %v", err)
	}
